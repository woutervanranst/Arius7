...
Arius.Core currently represents archive-relative paths, pointer paths, local filesystem paths, blob names, and cache paths with raw strings, which spreads normalization and `System.IO` usage across feature handlers and shared services. This makes cross-OS archive behavior harder to reason about and lets invalid paths travel too far before failing.

## What Changes

- Introduce filesystem domain types under `Arius.Core.Shared.FileSystem`, centered on a generic slash-normalized `RelativePath` value object.
...
- Refactor archive, list, restore, filetree, blob-name, and cache-path internals away from path-like raw strings, while allowing public contracts to expose `RelativePath` where that improves the API.
...
- `filesystem-domain-types`: Path and file-domain type system for Arius.Core, including `RelativePath`, `PathSegment`, pointer path derivation, archive-time file pairs, and local filesystem quarantine.
...
- `list-query`: Listing traversal and local/cloud merge use relative domain paths.
...
- Public contracts in Arius.Core may expose `RelativePath` and `PathSegment` where that aligns with the implemented API.
