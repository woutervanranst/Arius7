...
The system SHALL look up each content hash in the chunk index to determine the chunk hash and chunk type. For large files, the content-hash equals the chunk-hash. For tar-bundled files, the content-hash maps to a different chunk-hash (the tar). The system SHALL group all file entries by chunk hash to minimize downloads (multiple files from the same tar → one download).

#### Scenario: Large file chunk resolution
- **WHEN** content hash `abc123` maps to chunk hash `abc123` with `original-size: 50000000`
...
- **WHEN** content hash `def456` maps to chunk hash `tar789` with `original-size: 2048`
