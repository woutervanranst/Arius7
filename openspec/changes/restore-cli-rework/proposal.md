...
- **Log the silent fourth case**: When `!opts.Overwrite` and the file exists with a differing hash, the file is correctly NOT overwritten but no log message is emitted. Add `[disposition] {Path} -> keep (local differs, no --overwrite)` log. Capture in a test.
