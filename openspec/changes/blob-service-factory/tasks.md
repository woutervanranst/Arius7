...
- [x] 2.1 Create `BlobServiceFactory` class in `Arius.AzureBlob` with static `CreateAsync` method accepting `object` credential, account name, container name, and `PreflightMode`
...
- [x] 3.2 Slim `BuildProductionServices` to: resolve credential, call `BlobServiceFactory.CreateAsync`, build DI container with returned `IBlobStorageService`
