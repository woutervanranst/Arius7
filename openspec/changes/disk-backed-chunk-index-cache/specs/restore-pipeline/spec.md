...
SQLite and chunk-index local-store details SHALL remain hidden behind `ChunkIndexService`; restore SHALL depend only on chunk-index service APIs and SHALL NOT reference SQLite, `Microsoft.Data.Sqlite`, or chunk-index local-store internals directly.
