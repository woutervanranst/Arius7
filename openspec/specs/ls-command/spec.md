...
The system SHALL list all files in a snapshot by traversing the merkle tree. The output SHALL include relative path, content hash, and file metadata (created date, modified date). File size SHALL be retrieved from the chunk index. The default snapshot SHALL be the latest; `-v` SHALL select a specific version.

#### Scenario: List all files in latest snapshot
- **WHEN** `arius ls` is run without filters or `-v`
- **THEN** the system SHALL traverse the latest snapshot's tree and display all files with path, size, and dates
