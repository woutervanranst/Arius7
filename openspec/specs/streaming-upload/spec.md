# Streaming Upload Spec

## Purpose

Defines the streaming upload primitives for Arius: the `ProgressStream` read wrapper, the `CountingStream` write wrapper, the fully streaming upload chain for large files and tar bundles, and the post-upload metadata pattern.
