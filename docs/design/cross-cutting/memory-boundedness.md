...
- **The one channel that carries bulk bytes is bounded tightly.** `sealedTarChannel = Channel.CreateBounded<SealedTar>(TarUploadWorkers)` carries actual [tar chunk](../../glossary.md#tar-chunk) bytes, so it is bounded to the upload-worker count; the metadata-only channels (`filePairChannel`, `hashedChannel`, `chunkIndexEntryChannel`, `fileTreeEntryChannel`) carry small path/hash records and stay unbounded by design — `filePairChannel` in particular is deliberately unbounded so file *enumeration* can reach EOF, and publish the scan total, without being throttled to *hashing*'s pace.
...
- **The bulk-byte channel stays bounded; metadata channels may be unbounded.** Only `sealedTarChannel` carries a size-proportional payload and must stay `CreateBounded`. `filePairChannel` scales with file count in flight but carries no content (paths + optional hash only), so it stays unbounded on purpose; bounding any metadata channel would only add latency, not safety.
