...
`Program.cs` wires the whole host: it registers the app database, `SecretProtector`, the Azure cost estimator (`IStorageCostEstimator` → `AzureBlobCostEstimator`, a singleton — [cost estimation](../core/shared/cost.md)), the per-repo `RepositoryProviderRegistry`, the `JobRunner`, the `SchedulerService`, SignalR (camelCase payloads), CORS for the dev `ng serve` origin (`http://localhost:4200`), then static-file serving for the SPA. Routes split cleanly so client-side SPA routes never collide with the server: REST lives under `/api`, the hub under `/hubs/arius`, and `MapFallbackToFile("index.html")` serves the Angular app for everything else. An `/api`-group endpoint filter stamps every response with an `X-Arius-Version` header (`AriusVersion.Display`), CORS-exposed so the dev SPA can read it cross-origin.
...
Core registers everything as singletons **scoped to one repository** (one account / container / passphrase). The web host therefore cannot use a single global Core container — it manages many repositories at once. `RepositoryProviderRegistry` builds one `IServiceProvider` per repository on demand, each with its own `IMediator` + Core service graph (`BuildAsync` → `services.AddMediator(); services.AddArius(blobContainer, passphrase, account, container, costEstimator)`, the cost estimator being the app-singleton Azure implementation — [cost estimation](../core/shared/cost.md)). Connection material is loaded from the app DB and decrypted via `SecretProtector` just-in-time (`LoadConnection`).
...
`AppData/AppDatabase` is the host's own SQLite store — **separate from Core's chunk-index cache** — holding `storage_accounts`, `repositories`, `jobs`, `schedules`, and the `statistics_cache` (raw `SqliteConnection`, WAL, parameterized commands, mirroring Core's local-store idiom). It is the only persistent state the SPA reads through REST: account/repo CRUD (`AccountEndpoints` — incl. each account's Azure **region** (`storage_accounts.location`) and key rotation via `GET`/`PATCH`/`DELETE`, `RepositoryEndpoints`), the priced-region list for the account dropdown (`PricingEndpoints` → `/pricing/regions`, backed by `IStorageCostEstimator.Regions`), a server-side directory browser for the local-path picker (`FilesystemEndpoints` → `/fs/list`, listing folders as the **Api host/container** sees them), job history and cron schedules (`JobEndpoints`), and the snapshot/statistics reads behind `BrowseEndpoints` — `SnapshotsQuery` goes to Core live, while `StatisticsQuery` (now also carrying region-priced cost) is **memoized** in `statistics_cache` (below). Secrets — Azure account keys and repository passphrases — are stored as **ASP.NET Data Protection** ciphertext (`SecretProtector`, keyed by a key ring persisted to the mounted volume) and **never** serialized back to the client (the DTOs expose only `HasKey`, never the key/passphrase).
...
- **The statistics cache is invalidated on every snapshot-set change and every account-region change, never re-verified per read.** Whatever clears the read provider (archive job, properties change, delete) — plus an account region change, since the memoized cost is region-priced — must also `ClearStatisticsCache`; otherwise the SPA serves figures computed against a superseded snapshot generation or region.
