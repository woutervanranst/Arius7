...
- **The facade is the boundary.** `LookupAsync`/`AddEntry`/`FlushAsync`/`GetStatistics`/`GetDeduplicatedOriginalSize` are `internal` interface members; callers outside `Shared/ChunkIndex` never compute prefixes or touch the router/store directly. Architecture tests enforce this.
