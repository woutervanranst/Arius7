...
- **Single owner of local SQLite.** Only `ChunkIndexLocalStore` defines the schema, opens connections, and runs transactions; writes serialize on `_gate` plus Microsoft.Data.Sqlite busy-retry. SQLite calls are synchronous (SQLite has no real async I/O); async stays at the remote-I/O boundary.
