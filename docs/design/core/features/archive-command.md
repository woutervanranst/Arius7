...
**1 Enumerate** (`LocalFileEnumerator.Enumerate`, ×1) — single-pass depth-first walk via `RelativeFileSystem`. Recognizes `.pointer.arius` files, pairs each binary with its pointer (and vice versa) by *path derivation* — no dictionary or state tracking — and yields one `FilePair` per path. Broken symlinks and invalid pointer content are logged and skipped. Publishes `FileScannedEvent` per file and `ScanCompleteEvent` at the end.
