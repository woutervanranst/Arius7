...
> **Code:** `src/Arius.Core/Features/ArchiveCommand/`  ·  **Decisions:** [ADR-0006](../../../decisions/adr-0006-build-filetrees-from-hashed-directory-staging.md) · [ADR-0007](../../../decisions/adr-0007-separate-phase-and-detail-logging-in-pipeline-handlers.md) · [ADR-0017](../../../decisions/adr-0017-idempotent-non-distributed-recovery.md) · [ADR-0002](../../../decisions/adr-0002-skip-snapshots-for-no-op-archives.md) · [ADR-0019](../../../decisions/adr-0019-central-file-exclusion-configuration.md) · [ADR-0021](../../../decisions/adr-0021-opt-in-change-detection-hashcache.md)  ·  **Terms:** [content hash](../../../glossary.md#content-hash) · [chunk](../../../glossary.md#chunk) · [large chunk](../../../glossary.md#large-chunk) · [tar chunk](../../../glossary.md#tar-chunk) · [thin chunk](../../../glossary.md#thin-chunk) · [FilePair](../../../glossary.md#filepair) · [snapshot](../../../glossary.md#snapshot) · [hashcache](../../../glossary.md#hashcache) · [fast-hash](../../../glossary.md#fast-hash)
...
**4a Large Upload** (×4) — one [large chunk](../../../glossary.md#large-chunk) per file via `IChunkStorageService.UploadLargeAsync` (stream the file → compress → optionally encrypt → blob `chunks/<content-hash>`; for a large chunk, chunk hash == content hash). `opts.UploadTier` and `opts.SmallFileThreshold` are both passed: the tier is a *ceiling*, and storage keeps a chunk that compressed to within the threshold out of the archive tier ([chunk-storage](../shared/chunk-storage.md#the-archive-tier-ceiling)). Emits a `ShardEntry` carrying `uploadResult.ActualTier` — the tier the blob is really in, not the one requested — to `chunkIndexEntryChannel`, and the `HashedFilePair` to `fileTreeEntryChannel`.
...
- **The storage tier hint records where the blob actually is.** Stage 4a/4c write `uploadResult.ActualTier`, never `opts.UploadTier`. A chunk whose stored size is within `SmallFileThreshold` is kept online even under `--tier Archive`, and the index must say so — otherwise restore pays to rehydrate a blob it could download directly ([ADR-0023](../../../decisions/adr-0023-archive-tier-small-chunk-ceiling.md)).
...
- **Channel pipeline over a monolithic loop** — independent stages run concurrently at their own degree of parallelism, with backpressure isolated to the one channel that holds bytes (`sealedTarChannel`). Hashing (CPU) overlaps upload (network) overlaps local-state writes (disk/SQLite).
...
- **Routing is still decided on the uncompressed size.** A file above `SmallFileThreshold` that compresses back within it is correctly kept out of the archive tier by the ceiling in [chunk-storage](../shared/chunk-storage.md#the-archive-tier-ceiling), but it remains its own standalone blob rather than being bundled with its neighbours. Routing on the stored size would need a compressibility probe before stage 3 (one could ride the stage-2 read, which already streams every byte through `SparseSamplingStream`) *and* a `TarBuilder` whose memory is bounded by compressed rather than uncompressed bytes — see the next bullet.
- **`TarBuilder` seals on uncompressed size, which is what bounds its memory.** `TarTargetSize` caps RAM only because accumulation is uncompressed; sealing on the compressed size instead (as v5 Arius did, by writing the tar straight into a gzip stream) would need the bundle compressed inside `TarBuilder`, the raw bytes teed to a hasher to keep the tar chunk hash over the uncompressed body, and `UploadTarAsync` accepting already-compressed content.
