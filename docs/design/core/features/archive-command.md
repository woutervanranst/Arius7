...
- **`--remove-local` implies writing pointers.** The handler resolves `writePointers = opts.WritePointers || opts.RemoveLocal`. Removing the binary without a pointer would lose the path entirely. There is no mutual-exclusion rejection — `--remove-local` simply enables pointers regardless of whether `--write-pointers` was passed. The old `--no-pointers` flag on `archive` is gone; pointers are now **opt-in** (`--write-pointers`, default off).
