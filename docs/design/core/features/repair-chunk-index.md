...
flowchart TD
    ...
    recreate --> list["GetRepairEntriesAsync — list chunks-v5legacy-metadata/ then chunks/:<br/>descriptor = own metadata (has arius_type) ?? sidecar[hash]<br/>large → entry inline · thin → write-through (parent in chunk_hash + placeholder)<br/>tar → remember (tier, chunkSize), emit no entry · tier always from live listing"]
...
**Descriptor resolution — own metadata, or a v5-legacy sidecar.** Before dispatching on `arius_type`, `GetRepairEntriesAsync` first lists `BlobPaths.V5LegacySideCarPrefix` (`chunks-v5legacy-metadata/`) into a hash→metadata map, then resolves each chunk's descriptor as **its own metadata if that carries `arius_type`, otherwise the sidecar** for that hash. A chunk with neither is skipped with a warning (same rule as a missing sentinel). This exists for v5→v7-[migration](../../migration.md): an Archive-tier chunk cannot take `Set Blob Metadata`, so the migration parks its descriptor in a zero-byte Cool sidecar that repair reads back ([ADR-0018](../../../decisions/adr-0018-archive-tier-metadata-sidecar.md)). A native v7 repo has no sidecars, so this is one empty listing. The descriptor only ever supplies `arius_type`/sizes/parent — the **`StorageTierHint` always comes from the live `chunks/` listing**, never the sidecar (a sidecar is written once and cannot track later rehydration).
...
- **A descriptor may come from a sidecar, but the tier never does.** When a chunk's own metadata lacks `arius_type`, repair reads the descriptor from its `chunks-v5legacy-metadata/` sidecar ([ADR-0018](../../../decisions/adr-0018-archive-tier-metadata-sidecar.md)); the `StorageTierHint` is still taken from the live `chunks/` listing, so a chunk rehydrated after migration is recorded at its current tier, not the tier frozen in the sidecar.
