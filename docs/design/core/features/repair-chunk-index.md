...
- **Metadata may come from a sidecar, but the tier never does.** When a chunk's own metadata lacks `arius_type`, repair reads its metadata from the `chunks-v5legacy-metadata/` sidecar ([ADR-0018](../../../decisions/adr-0018-archive-tier-metadata-sidecar.md)); the `StorageTierHint` is still taken from the live `chunks/` listing, so a chunk rehydrated after migration is recorded at its current tier, not the tier frozen in the sidecar.
