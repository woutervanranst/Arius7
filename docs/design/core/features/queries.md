...
`StatisticsQuery(Version? = null, EnsureFullCoverage = false, Region? = null)` is an `IQuery<RepositoryStatistics>` returning `RepositoryStatistics(Files, OriginalSize, DeduplicatedSize, StoredSize, UniqueChunks, Region, TotalStorageCostPerMonth, StoredByTier)` (costs in EUR). It joins three sources across **two scopes** plus a cost layer:
