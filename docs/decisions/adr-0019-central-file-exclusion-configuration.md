...
`src/Arius.Core/appsettings.json` is an `<EmbeddedResource>` holding the `Arius:Exclusions` section. `AddArius` layers it as the base configuration, optionally over a host `IConfiguration`, binds it to `FileExclusionOptions` via the options pattern, and registers a `FileExclusionFilter` singleton consulted by both the archive enumeration (`LocalFileEnumerator.EnumerateAsync`) and the `ls` local overlay (`LocalDirectoryReader.Read`), so a listing never shows an excluded file as local-only. Both types live in `Shared/FileSystem` (shared filesystem policy, not an archive-only concern):
