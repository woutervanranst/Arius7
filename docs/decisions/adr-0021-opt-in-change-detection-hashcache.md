...
3. **Warm-cache run** — immediately run `arius archive <path> --fast-hash` against the same unchanged dataset. Record the same metrics. The `[archive] Done: … fasthash-reused=… fasthash-rehashed=…` summary log (or `ArchiveResult.FastHashReused` / `FastHashRehashed`) shows the cache hit rate.
