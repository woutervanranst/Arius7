...
- Extract a new `AzureBlobServiceFactory` class into `Arius.AzureBlob` that encapsulates:
...
- Slim down `CliBuilder.BuildProductionServices` to: resolve credential → call `AzureBlobServiceFactory.CreateAsync(...)` → build DI container
...
- **Arius.AzureBlob** — new `AzureBlobServiceFactory` class, new `Azure.Identity` dependency, `PreflightMode` enum and `PreflightException` class move here
