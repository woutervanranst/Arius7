...
- **Pointer sidecars are opt-in.** `--write-pointers` (default off) writes `.pointer.arius` sidecars; `--remove-local` requires `--write-pointers` (cannot remove the binary without leaving a local record).
...
- **Hosts talk only to Features, never to Core `Shared` services directly.** A host (`Arius.Cli`, `Arius.Explorer`, `Arius.Api`) may send/stream commands and queries through `IMediator`, but must not resolve or call Core `Shared` services (`IChunkStorageService`, `ISnapshotService`, `IChunkIndexService`, `IFileTreeService`, `IBlobContainerService`, …). If a host needs data or an action that no feature exposes, add or extend a feature — do not reach into `Shared`. Example: rehydration progress is obtained by re-triggering the restore feature (which is idempotent), not by calling `IChunkStorageService.ListRehydratedChunksAsync` from the Api.
