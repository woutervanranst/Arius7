...
Invoke the repository's available skills before .NET/C#/testing/package/configuration work when they match the task.
...
- Design must account for large scale: terabytes of binary data and thousands of small files. Consider both byte scale and file-count scale when designing or optimizing features.
- Prefer streaming, batching, and bounded-memory or bounded-disk pipelines over whole-repository in-memory materialization when file count/size can grow.
