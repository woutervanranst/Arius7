...
- **Pointer sidecars are opt-in.** `--write-pointers` (default off) writes `.pointer.arius` sidecars; `--remove-local` requires `--write-pointers` (cannot remove the binary without leaving a local record) — the handler rejects `--remove-local` without `--write-pointers` up front. The old `--no-pointers` flag on `archive` is gone.
