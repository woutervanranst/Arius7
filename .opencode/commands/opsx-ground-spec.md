...
6. **Rewrite active change artifacts only, clairvoyantly**
   ...
   - `specs/<capability>/spec.md` — requirements and scenarios that match the real behavior, preserving the ADDED/MODIFIED/REMOVED/RENAMED delta format. Delete requirements/scenarios for behavior that was not built. Rewrite (do not annotate) requirements whose behavior shipped differently. Schema blocks, API signatures, and names must match the code exactly.
     - **Delete the whole `specs/<capability>/` delta when that capability was not actually modified by this change.** If the delta prescribes changes but the capability's code and externally observable behavior are unchanged from the baseline — it only consumes another capability's facade, or its diff is incidental (DI wiring, refactor, cosmetic) — remove the delta directory entirely and drop the capability from the proposal's capabilities list. A delta must correspond to a real modification; a clairvoyant spec would never have listed an untouched capability.
     - **Add a `specs/<capability>/` delta when the code modified a capability that has none.** If implementation changed a capability's observable behavior or contract and no delta captures it, create the delta with the correct ADDED/MODIFIED/REMOVED sections and add the capability to the proposal's list. If unsure which capability owns it, ask (per step 7).
