...
public sealed class ArchiveCommandHandler : ICommandHandler<ArchiveCommand, ArchiveResult>
{
    ...
    public async ValueTask<ArchiveResult> Handle(ArchiveCommand command, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var             pendingPointers = new ConcurrentBag<(RelativePath RelativePath, ContentHash Hash)>();
            ...
            var enumTask = Task.Run(async () =>
            {
                try
                {
                    ...
                    {
                        count++;
                        var fullPath = pair.BinaryExists ? opts.RootDirectory / pair.RelativePath : default(RootedPath?);
                        var fileSize = fullPath is { } path && path.ExistsFile ? path.Length : 0L;
                        totalBytes += fileSize;
            ...
            var hashTask = Parallel.ForEachAsync(
                    ...
                    async (pair, ct) =>
                    {
                        ...
                        var fullBinaryPath = pair.BinaryExists ? opts.RootDirectory / pair.RelativePath : default(RootedPath?);
                        var fileSize = fullBinaryPath is { } path ? path.Length : 0L;
                        ...
                        {
                            await using var fs           = fullBinaryPath!.Value.OpenRead();
                            var             hashProgress = opts.CreateHashProgress?.Invoke(relativePathText, fileSize) ?? new Progress<long>();
            ...
            var dedupTask = Task.Run(async () =>
            {
                try
                {
                    ...
                    {
                        var relativePathText = hashed.FilePair.RelativePath.ToString();
                        ...
                        else
                        {
                            ...
                            _logger.LogInformation("[dedup] {Path} -> new/{Route} ({Hash}, {Size})", relativePathText, route, hashed.ContentHash.Short8, fileSize.Bytes().Humanize());
            ...
            var largeUploadTask = Parallel.ForEachAsync(
                ...
                async (upload, ct) =>
                {
                    var relativePathText = upload.HashedPair.FilePair.RelativePath.ToString();
                    ...
                    var fullPath = opts.RootDirectory / upload.HashedPair.FilePair.RelativePath;
            ...
            if (opts.RemoveLocal)
            {
                ...
                {
                    var path = opts.RootDirectory / relativePath;
                    try
                    {
                        path.DeleteFile();
                    }
    ...
    private static async Task WriteFileTreeEntry(HashedFilePair hashed, LocalRootPath rootDirectory, FileTreeStagingWriter writer, CancellationToken ct)
    {
        ...
        {
            var fullPath = rootDirectory / pair.RelativePath;
            created  = new DateTimeOffset(fullPath.CreationTimeUtc,  TimeSpan.Zero);
            modified = new DateTimeOffset(fullPath.LastWriteTimeUtc, TimeSpan.Zero);
        }
        else
        {
