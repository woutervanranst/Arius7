...
public class RepositoryExplorerViewModelTests
{
    ...
    public async Task Constructor_WhenRecentRepositoryExists_LoadsRepositoryTree()
    {
        ...
        mediator.CreateStream(Arg.Any<ListQuery>(), Arg.Any<CancellationToken>())
            .Returns(_ => ToAsyncEnumerable<RepositoryEntry>(
                new RepositoryDirectoryEntry(PathOf("folder1/folder2"), TreeHashD, true, true),
                new RepositoryFileEntry(PathOf("folder1/file-a.txt"), ContentHashA, 1024, null, null, true, true, true, true, null),
                new RepositoryFileEntry(PathOf("folder1/file-b.txt"), null, 2048, null, null, false, true, false, true, null)));
        mediator.CreateStream(Arg.Any<ChunkHydrationStatusQuery>(), Arg.Any<CancellationToken>())
            .Returns(_ => ToAsyncEnumerable(
                new ChunkHydrationStatusResult(PathOf("folder1/file-a.txt"), ContentHashA, ChunkHydrationStatus.Available)));
    ...
    public async Task RestoreCommand_WhenUserConfirms_RestoresFilesAndRefreshesNode()
    {
        ...
        mediator.CreateStream(Arg.Any<ListQuery>(), Arg.Any<CancellationToken>())
            .Returns(_ => ToAsyncEnumerable<RepositoryEntry>(
                new RepositoryFileEntry(PathOf("file-a.txt"), ContentHashA, 1024, null, null, true, true, true, true, true)));
        mediator.CreateStream(Arg.Any<ChunkHydrationStatusQuery>(), Arg.Any<CancellationToken>())
            .Returns(_ => EmptyAsyncEnumerable<ChunkHydrationStatusResult>());
        ...
        var node = new TreeNodeViewModel(RelativePath.Root, showPlaceholder: false)
        {
            ...
            {
                new(new RepositoryFileEntry(PathOf("file-a.txt"), ContentHashA, 1024, null, null, true, true, true, true, true))
            }
        };
