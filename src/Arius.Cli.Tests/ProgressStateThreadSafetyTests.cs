...
public class ProgressStateThreadSafetyTests
{
    ...
    public async Task ConcurrentAddAndRemove_NoDataRaces()
    {
        ...
        await Parallel.ForEachAsync(
            ...
            (i, _) =>
            {
                var path = RelativePath.Parse($"file{i}.bin");
    ...
    public async Task ConcurrentIncrements_FilesHashed_CorrectTotal()
    {
        ...
        for (var i = 0; i < n; i++)
            state.AddFile(RelativePath.Parse($"file{i}"), 100);
        ...
        await Parallel.ForEachAsync(
            ...
            (i, _) =>
            {
                state.SetFileHashed(RelativePath.Parse($"file{i}"), ContentHash.Parse(i.ToString("x64")));
                return ValueTask.CompletedTask;
            });
