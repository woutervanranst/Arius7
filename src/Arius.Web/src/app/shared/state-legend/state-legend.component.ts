...
@Component({
  ...
  template: `
    <div style="position:relative">
      <button class="flex items-center gap-1.5" data-testid="legend-button" style="font-size:12.5px;color:#71717a" (click)="open.set(!open())">
        <arius-state-ring [size]="15" [colors]="{ lo:'#27272a', li:'#2563eb', ro:'#27272a', ri:'#2563eb' }" />
        State legend
      </button>
      @if (open()) {
        <div data-testid="legend-popover" style="position:absolute;bottom:28px;right:0;z-index:30;width:300px;background:#fff;border:1px solid #e4e4e7;border-radius:12px;box-shadow:0 12px 32px rgba(9,9,11,.18);padding:16px">
