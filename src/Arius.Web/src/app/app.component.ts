    ...
    <div class="flex flex-col grow overflow-hidden bg-white"
      ...
      <header class="flex items-center justify-between shrink-0 px-6"
        ...
        <label class="flex items-center gap-2 px-3"
               style="width:300px;height:38px;background:#f4f4f5;border-radius:9px;color:#71717a">
          <i class="ki-filled ki-magnifier" style="font-size:16px"></i>
          <input type="text" placeholder="Search files across repositories…"
                 class="grow bg-transparent outline-none text-[13.5px]" />
...
export class AppComponent {
  ...
  protected crumb(): string {
    const segment = this.router.url.split('/').filter(Boolean)[0] ?? 'overview';
