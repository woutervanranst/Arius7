...
public class FileTreeBuilderTests
{
    ...
    {
        await FileTreePaths.GetStagingNodePath(stagingRoot, directoryId).WriteAllLinesAsync(lines);
    }
    ...
    public async Task SynchronizeAsync_IdenticalManifest_SameRootHash()
    {
        ...
        if (cache1.ExistsDirectory) cache1.DeleteDirectory(recursive: true);
        if (cache2.ExistsDirectory) cache2.DeleteDirectory(recursive: true);
