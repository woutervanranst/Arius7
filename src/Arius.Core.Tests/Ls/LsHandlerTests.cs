...
public class LsHandlerTests
{
    ...
    public async Task Handle_CloudOnlyNonRecursive_StreamsRootDirectoryEntries()
    {
        ...
        await foreach (var entry in handler.Handle(new LsCommand(new LsOptions { Recursive = false }), CancellationToken.None))
        {
            results.Add(entry);
        }
    ...
    public async Task Handle_PrefixAndNonRecursive_StreamsOnlyImmediateChildrenOfPrefix()
    {
        ...
        var results = new List<RepositoryEntry>();
        await foreach (var entry in handler.Handle(new LsCommand(new LsOptions { Prefix = "docs", Recursive = false }), CancellationToken.None))
        {
            results.Add(entry);
        }
    ...
    public async Task Handle_WithLocalPath_MergesCloudAndLocalFilesInOneDirectory()
    {
        ...
        try
        {
            ...
            await foreach (var entry in handler.Handle(new LsCommand(new LsOptions { LocalPath = tempRoot, Recursive = false }), CancellationToken.None))
            {
    ...
    public async Task Handle_RecursiveFalse_YieldsOnlyImmediateChildren()
    {
        ...
        var nonRecursive = await CollectAsync(handler.Handle(new LsCommand(new LsOptions { Recursive = false }), CancellationToken.None));
        ...
        var recursive = await CollectAsync(handler2.Handle(new LsCommand(new LsOptions { Recursive = true }), CancellationToken.None));
    ...
    public async Task Handle_FilenameFilter_CaseInsensitiveMatchOnFilesNotDirs()
    {
        ...
        var results = await CollectAsync(handler.Handle(new LsCommand(new LsOptions { Recursive = false, Filter = "vacation" }), CancellationToken.None));
    ...
    public async Task Handle_DirectoryMerge_AllThreeKindsYieldedWithCorrectFlags()
    {
        ...
        try
        {
            ...
            var results = await CollectAsync(handler.Handle(new LsCommand(new LsOptions { Recursive = false, LocalPath = tempRoot }), CancellationToken.None));
    ...
    public async Task Handle_BatchSizeLookup_CalledOncePerDirectory_SizeNullWhenNotInIndex()
    {
        ...
        var handler = MakeHandler(blobs, index);
        var results = await CollectAsync(handler.Handle(new LsCommand(new LsOptions { Recursive = true }), CancellationToken.None));
    ...
    public async Task Handle_NoSnapshots_ThrowsInvalidOperationException()
    {
        ...
        {
            await foreach (var _ in handler.Handle(new LsCommand(new LsOptions()), CancellationToken.None)) { }
        });
    ...
    public async Task Handle_SpecificVersionNotFound_ThrowsWithDescriptiveMessage()
    {
        ...
        var ex = await Should.ThrowAsync<InvalidOperationException>(async () =>
        {
            await foreach (var _ in handler.Handle(new LsCommand(new LsOptions { Version = "9999-not-found" }), CancellationToken.None)) { }
        });
    ...
    public async Task Handle_CancellationRequested_StopsEnumeration()
    {
        ...
        {
            await foreach (var entry in handler.Handle(new LsCommand(new LsOptions { Recursive = true }), cts.Token))
            {
                collected.Add(entry);
    ...
    private LsHandler MakeHandler(FakeBlobService blobs, ChunkIndexService index) =>
        new(blobs, s_encryption, index, NullLogger<LsHandler>.Instance, "account", "container");
