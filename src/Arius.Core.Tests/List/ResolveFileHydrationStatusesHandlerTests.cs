...
public class ResolveFileHydrationStatusesHandlerTests
{
    ...
    {
        foreach (var testCase in StatusCases())
        {
            var largeContentHash = HashFor($"large-content-{testCase.Name}");
            var tarChunkHash     = HashFor($"tar-chunk-{testCase.Name}");
            var thinContentHash  = HashFor($"thin-content-{testCase.Name}");
            var tarContentHash   = HashFor($"tar-content-{testCase.Name}");

            var blobs = new FakeBlobContainerService();
            testCase.ConfigureChunk(blobs, largeContentHash, BlobMetadataKeys.TypeLarge);
            testCase.ConfigureChunk(blobs, tarChunkHash, BlobMetadataKeys.TypeTar);

            using var index = new ChunkIndexService(blobs, s_encryption, $"acct-hydration-{testCase.Name}", $"ctr-hydration-{testCase.Name}", cacheBudgetBytes: 1024 * 1024);
            index.RecordEntry(new ShardEntry(largeContentHash, largeContentHash, 100, 25));
            ...
            var files = new[]
            {
                new RepositoryFileEntry("large.bin", largeContentHash, 100, null, null, true, false, null, null),
            ...
            results.ShouldContain(result => result.RelativePath == "large.bin" && result.Status == testCase.ExpectedStatus);
            results.ShouldContain(result => result.RelativePath == "thin.bin" && result.Status == testCase.ExpectedStatus);
            results.ShouldContain(result => result.RelativePath == "tar.bin" && result.Status == testCase.ExpectedStatus);
        }
    }
    ...
    private static IEnumerable<HydrationStatusCase> StatusCases()
    {
        yield return new HydrationStatusCase(
            "non-archive",
