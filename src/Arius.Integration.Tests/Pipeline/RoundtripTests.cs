...
public class RoundtripTests(AzuriteFixture azurite)
{
    ...
    public async Task Archive_Incremental_EachSnapshotVersion_CorrectContent()
    {
        ...
        var restoreResult1 = await fix.CreateRestoreHandler().Handle(
            ...
            {
                RootDirectory = fix.RestoreRoot.GetSubdirectoryRoot(PathSegment.Parse("v1")),
                Version       = snapshot1,
                Overwrite     = true,
            }), default);
        ...
        var v1Dir = fix.RestoreRoot.GetSubdirectoryRoot(PathSegment.Parse("v1"));
        ...
        var v2Dir = fix.RestoreRoot.GetSubdirectoryRoot(PathSegment.Parse("v2"));
    ...
    public async Task Archive_FileDeleted_AbsentFromNewSnapshot_PresentInOld()
    {
        ...
        var latestDir = fix.RestoreRoot.GetSubdirectoryRoot(PathSegment.Parse("latest"));
